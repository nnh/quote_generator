...
class Add_del_columns {
  ...
  get_setup_closing_range() {
    try {
      ...
      const header_t = this.sheet
        .getRange(this.term_row, 1, 1, lastColumn)
        .getValues()[0];
      ...
      if (setup_idx < 0 || closing_idx < 0) {
        console.warn(
          `Setup sheet (${this.configCache.setupSheetName}) or Closing sheet (${this.configCache.closingSheetName}) not found in header`
        );
        return null;
      }
  ...
  remove_col() {
    try {
      ...
      let duplication_col_numbers = duplication_cols
        .map((x) => {
      ...
      const batchGroups = this._groupConsecutiveColumns(
        duplication_col_numbers,
        false
      );
      ...
      batchGroups.forEach((group) => {
        ...
        if (startCol > 0 && startCol <= this.sheet.getLastColumn()) {
          ...
          console.warn(
            `Invalid column range for batch deletion: start=${startCol}, count=${group.count}`
          );
        }
      });
    } catch (error) {
    ...
    try {
      ...
      batchGroups.reverse().forEach((group) => {
        if (group.start > 0 && group.start <= this.sheet.getLastColumn()) {
          ...
          console.warn(
            `Invalid column range for batch deletion: start=${group.start}, count=${group.count}`
          );
        }
      });
    } catch (error) {
  ...
  set add_target(target_array) {
    try {
      if (
        !target_array ||
        !Array.isArray(target_array) ||
        target_array.length < 3
      ) {
      ...
      if (
        !this.target_head ||
        typeof this.target_columns_count !== "number" ||
        this.target_columns_count <= 0
      ) {
        console.error(
          "Invalid target_head or target_columns_count in target_array"
        );
  ...
  add_cols() {
    try {
      ...
      const columns_count = header_t.filter(
        (x) => x == this.target_head
      ).length;
