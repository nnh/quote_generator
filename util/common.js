...
function getColumnString(column_number) {
  ...
  const temp_res = temp_range.getA1Notation().replace(/\d/, "");
  return temp_res;
}
/**
 * 開始日、終了日から月数・年数を返す
...
function get_years(start_date, end_date) {
  ...
  const temp = get_months(start_date, end_date);
...
function get_sheets() {
  ...
  let sheet = {
    trial: ss.getSheetByName(
      cache.scriptProperties.getProperty("trial_sheet_name")
    ),
    quotation_request: ss.getSheetByName(
      cache.scriptProperties.getProperty("quotation_request_sheet_name")
    ),
    ...
    registration_2: ss.getSheetByName(
      cache.scriptProperties.getProperty("registration_2_sheet_name")
    ),
    interim_1: ss.getSheetByName(
      cache.scriptProperties.getProperty("interim_1_sheet_name")
    ),
    observation_1: ss.getSheetByName(
      cache.scriptProperties.getProperty("observation_1_sheet_name")
    ),
    interim_2: ss.getSheetByName(
      cache.scriptProperties.getProperty("interim_2_sheet_name")
    ),
    observation_2: ss.getSheetByName(
      cache.scriptProperties.getProperty("observation_2_sheet_name")
    ),
    closing: ss.getSheetByName(cache.closingSheetName),
    items: ss.getSheetByName(
      cache.scriptProperties.getProperty("items_sheet_name")
    ),
    quote: ss.getSheetByName(cache.quoteSheetName),
    check: ss.getSheetByName(
      cache.scriptProperties.getProperty("value_check_sheet_name")
    ),
  };
