...
function copy_formulas_to_inserted_line(sheet, target_row, ref_row) {
  ...
  sheet
    .getRange(target_ref_row, 1, 1, last_col)
    .copyTo(
      sheet.getRange(target_row, 1),
...
function reconfigure_total(array_sheet, target_row) {
  ...
  for (var j = 0; j < array_sheet.length; j++) {
    trial_cell_addr = sheet.trial
      .getRange(parseInt(const_trial_setup_row) + j, const_trial_years_col)
      .getA1Notation();
    temp_str =
      temp_str +
      array_sheet[j] +
  ...
  // 最後の＋を消してセット
  sheet.total
    .getRange(target_row, const_count_col)
    .setFormula(temp_str.substr(0, temp_str.length - 1));
}
/**
 * 行の追加call
 */
