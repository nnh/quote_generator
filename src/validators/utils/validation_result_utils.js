 ...
 */
function validationBuildNgMessage_(message) {
 ...
 */
function validationToStatusFromBooleanArray_(
  booleanResults,
  ngReason,
  label = "",
) {
  const status = booleanResults.every(Boolean)
    ...
    : validationBuildNgMessage_(ngReason || VALIDATION_MESSAGES.VALUE_MISMATCH);
 ...
 *
 * checkFunc の実行結果（boolean配列）を `validationToStatusFromBooleanArray_` に渡し、
...
function validationBuildMessage_(checkFunc, message) {
  const status = validationToStatusFromBooleanArray_(
    checkFunc(),
