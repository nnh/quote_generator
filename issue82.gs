function issue82() {
  ...
  itemsSheet.getRange('B19').setValue('薬剤対応');
  ...
  itemsSheet.getRange('B39').setFormula(`=if(and('Quotation Request'!$A$2<>"", or(Trial!$B$27="医師主導治験", Trial!$B$27="先進")),"初期アカウント設定（施設・ユーザー）","初期アカウント設定（施設・ユーザー）、IRB承認確認")`);  
}
  ...
  const res = trialTypeList.map(trialType => {
    ...
    if (trialType === '医師主導治験' || trialType === '先進'){
      return itemsSheet.getRange('B16').getValue() === 'IRB承認確認、施設管理' &&
             itemsSheet.getRange('B19').getValue() === '薬剤対応' &&
             itemsSheet.getRange('B39').getValue() === '初期アカウント設定（施設・ユーザー）' &&
      ...
      return itemsSheet.getRange('B16').getValue() === 'IRB準備・承認確認' &&
             itemsSheet.getRange('B19').getValue() === '薬剤対応' &&
             itemsSheet.getRange('B39').getValue() === '初期アカウント設定（施設・ユーザー）、IRB承認確認' &&
