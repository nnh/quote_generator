 ...
 */
function processAndWriteTrialPeriod_(
  sheet,
  ...
  for (let j = 0; j < array_trial_date.length; j++) {
  ...
  sheet.trial
    ...
    .setFormula(
      "=datedif(" +
        sheet.trial.getRange(save_row, const_trial_start_col).getA1Notation() +
        ",(" +
        sheet.trial.getRange(save_row, const_trial_end_col).getA1Notation() +
        '+1),"m")'
    );
  sheet.trial
...
function get_setup_closing_term_(temp_str, array_quotation_request) {
  ...
  if (!cache.isValid) {
    console.error(
      "Failed to initialize ConfigCache in get_setup_closing_term_"
    );
    return;
  }

  let setup_term = QuoteScriptConstants.SETUP_TERM_SHORT;
  let closing_term = QuoteScriptConstants.CLOSING_TERM_SHORT;

  if (
    temp_str == cache.investigatorInitiatedTrial ||
    temp_str == cache.specifiedClinicalTrial
  ) {
  ...
  if (
    get_quotation_request_value(
      array_quotation_request,
      QuoteScriptConstants.RESEARCH_REPORT_SUPPORT
 ...
 * @example
 *   const array_trial_date_ = get_trial_start_end_date(trial_start_date, trial_end_date);
 */
function get_trial_start_end_date_(
  ...
  if (!cache.isValid) {
    console.error(
      "Failed to initialize ConfigCache in get_trial_start_end_date_"
    );
    return;
  }

  const normalizedDates = normalizeTrialDates_(
    input_trial_start_date,
    input_trial_end_date,
    cache
  );
  const setupClosingDates = calculateSetupClosingDates_(
    normalizedDates.trial_start_date,
    normalizedDates.trial_end_date,
    cache
  );
  const registrationDates = calculateRegistrationPeriods_(
    setupClosingDates,
    normalizedDates.trial_start_date,
    normalizedDates.trial_end_date
  );
...
function normalizeTrialDates_(
  ...
  cache.scriptProperties.setProperty(
    "trial_start_date",
    trial_start_date.format()
  );
...
function calculateSetupClosingDates_(trial_start_date, trial_end_date, cache) {
  const setup_start_date = trial_start_date
    .clone()
    .subtract(parseInt(cache.setupTerm), "months");
  ...
  const closing_end_date = trial_end_date
    .clone()
    .add(1, "days")
    .add(parseInt(cache.closingTerm), "months")
    .subtract(1, "days");
...
function calculateRegistrationPeriods_(
  ...
  const registration_1_dates = calculateRegistration1Period_(
    setup_end_date,
    diff_from_setup_end_to_closing_start
  );
  const observation_2_dates = calculateObservation2Period_(
    closing_start_date,
    registration_1_dates,
    diff_from_setup_end_to_closing_start
  );
  const registration_2_dates = calculateRegistration2Period_(
    registration_1_dates,
    observation_2_dates
  );
 ...
 */
function calculateRegistration1Period_(
  setup_end_date,
  diff_from_setup_end_to_closing_start
) {
  const start =
    diff_from_setup_end_to_closing_start > 0
      ...
      : "";
  const end =
    start !== "" ? start.clone().add(1, "years").subtract(1, "days") : "";
 ...
 */
function calculateObservation2Period_(
  closing_start_date,
  registration_1_dates,
  diff_from_setup_end_to_closing_start
) {
  const diff_from_reg1_end_to_closing_start =
    registration_1_dates.end !== ""
      ? closing_start_date.diff(registration_1_dates.end, "months")
      : 0;
  ...
  const end =
    diff_from_reg1_end_to_closing_start > 0
      ...
      : "";
  const start =
    end !== "" ? closing_start_date.clone().subtract(1, "years") : "";
 ...
 */
function calculateRegistration2Period_(
  registration_1_dates,
  observation_2_dates
) {
  const diff_from_reg1_end_to_obs2_start =
    observation_2_dates.start !== ""
      ? observation_2_dates.start.diff(registration_1_dates.end, "months")
      : 0;

  const end =
    diff_from_reg1_end_to_obs2_start > 0
      ? observation_2_dates.start.clone().subtract(1, "days")
      : "";
  const start =
    end !== "" ? registration_1_dates.end.clone().add(1, "days") : "";
...
function storeRegistrationYears_(registrationDates, normalizedDates, cache) {
  const registration_start_date = determineRegistrationStartDate_(
    registrationDates,
    normalizedDates.trial_start_date
  );
  const registration_end_date = determineRegistrationEndDate_(
    registrationDates,
    normalizedDates.trial_end_date
  );
  ...
  cache.scriptProperties.setProperty(
    "registration_years",
    get_years(registration_start_date, registration_end_date)
  );
}
...
function buildTrialDateArray_(setupClosingDates, registrationDates) {
  return [
    ...
    [
      registrationDates.registration_1_start_date,
      registrationDates.registration_1_end_date,
    ],
    [
      registrationDates.registration_2_start_date,
      registrationDates.registration_2_end_date,
    ],
    ["", ""],
    ["", ""],
    ["", ""],
    [
      registrationDates.observation_2_start_date,
      registrationDates.observation_2_end_date,
    ],
