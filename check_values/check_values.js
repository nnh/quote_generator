...
function check_output_values() {
  ...
  const trialPeriods = calculateTrialPeriods(
    array_quotation_request,
    cache,
    sheet,
    output_row
  ...
  // Build check items
  const checkItemsResult = buildCheckItems(
    array_quotation_request,
    cache,
    sheet,
    trialPeriods,
 ...
 */
function calculateTrialPeriods(
  array_quotation_request,
  cache,
  sheet,
  output_row
  ...
  sheet.check
    .getRange(
      output_row,
      output_col,
      trial_start_end.length,
      trial_start_end[0].length
    )
  ...
  if (
    (get_quotation_request_value(array_quotation_request, "試験種別") ==
      cache.investigatorInitiatedTrial) |
    (get_quotation_request_value(array_quotation_request, "試験種別") ==
  ...
  const trial_year =
    trial_months > MONTHS_PER_YEAR
      ? Math.trunc(trial_months / MONTHS_PER_YEAR)
      : "";
...
function validateTotalAmounts(sheet, output_row) {
  const total_total_amount = get_total_amount({
    sheet: sheet.total,
    item_cols: "B:B",
  ...
  const total2_total_amount = get_total_amount({
    sheet: sheet.total2,
    item_cols: "B:B",
    total_row_itemname: "合計",
    header_row: 4,
  ...
  const total3_total_amount = get_total_amount({
    sheet: sheet.total3,
    item_cols: "B:B",
    total_row_itemname: "合計",
    header_row: 3,
...
function buildCheckItems(
  ...
  total_checkitems.push({
    itemname: "プロトコルレビュー・作成支援",
    value: PROTOCOL_REVIEW_VALUE,
  });
  total_checkitems.push({
    itemname: "検討会実施（TV会議等）",
    value: MEETING_COUNT,
  });

  let temp_name;
  let temp_value;
  addConditionalCheckItem(
    total_checkitems,
    "PMDA相談資料作成支援",
    array_quotation_request,
    "PMDA相談資料作成支援"
  ...
  if (
    ...
    (get_quotation_request_value(
      ...
      "あり") |
    (get_quotation_request_value(array_quotation_request, cache.coefficient) ==
  ...
  if (
    ...
    temp_value = get_quotation_request_value(
      array_quotation_request,
      "中間解析に必要な図表数"
    );
  } else {
  ...
  temp_name = "試験開始準備費用";
  if (
    get_quotation_request_value(array_quotation_request, temp_name) == "あり"
  ) {
    temp_value = facilities_value;
  } else {
    temp_value = "";
  ...
  total_checkitems.push({ itemname: temp_name, value: temp_value });
  total_amount_checkitems.push({
    itemname: temp_name,
    value: temp_total_amount,
  });
  ...
  temp_name = "治験薬管理（中央）";
  if (
    get_quotation_request_value(array_quotation_request, "治験薬管理") == "あり"
  ) {
    temp_value = 1;
  } else {
    temp_value = "";
  ...
  if (
    ...
    total_amount = get_quotation_request_value(
      array_quotation_request,
      "研究協力費、負担軽減費"
    );
  } else {
...
function performFinalValidation(sheet, checkItemsResult, output_row) {
  const { total_checkitems, total_amount_checkitems, target_total } =
    checkItemsResult;
  ...
  const res_total_amount = total_amount_checkitems.map(
    (total_amount_checkitems) =>
      check_itemName_and_value(
        target_total_amount,
        total_amount_checkitems.itemname,
        total_amount_checkitems.value
      )
  );
 ...
 */
function addConditionalCheckItem(
  checkItems,
  itemName,
  requestArray,
  conditionKey,
  ...
  const temp_value =
    get_quotation_request_value(requestArray, conditionKey) === "あり"
      ? trueValue
      : falseValue;
 ...
 */
function addTrialTypeCheckItem(
  checkItems,
  itemName,
  requestArray,
  cache,
  ...
  const temp_value =
    trialType === cache.investigatorInitiatedTrial
      ? investigatorValue
      : otherValue;
  checkItems.push({ itemname: itemName, value: temp_value });
  return temp_value;
