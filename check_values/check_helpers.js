function addTrialTypeConditionalItem(
  checkItems,
  investigatorName,
  otherName,
  requestArray,
  ...
  const isInvestigator =
    trialType === properties.getProperty("investigator_initiated_trial");
...
function addThresholdCheckItem(
  checkItems,
  itemName,
  requestArray,
  conditionKey,
...
function addMonitoringCheckItem(
  checkItems,
  itemName,
  requestArray,
  facilitiesValue
...
function addAnalysisCountItem(checkItems, itemName, requestArray, conditions) {
  ...
  conditions.forEach((condition) => {
    if (
      get_quotation_request_value(requestArray, condition) === CONDITION_ARI
    ) {
      value++;
    }
  });
...
function addDelegateCheckItem(
  ...
  const value =
    get_quotation_request_value(requestArray, conditionKey) === SETUP_DELEGATE
      ? trialMonths
      : "";
  checkItems.push({ itemname: itemName, value: value });
  return value;
...
function addYearBasedItem(
  ...
  if (
    get_quotation_request_value(requestArray, conditionKey) === CONDITION_ARI
  ) {
