...
function get_trial_start_end_date(trial_start_date, trial_end_date){
  ...
  // setup開始日
  var setup_start_date = trial_start_date.clone().subtract(parseInt(get_s_p.getProperty('setup_term')), 'months');
  // setupシートの最終日はsetup開始年度の3/31
  var setup_end_date = Moment.moment([setup_start_date.clone().subtract(3, 'months').year()　+ 1, 2, 31]);
  // closing終了日
  var closing_end_date = trial_end_date.clone().add(parseInt(get_s_p.getProperty('closing_term')), 'months');
  // closingシートの開始日はclosing終了年度の4/1
  var closing_start_date = Moment.moment([closing_end_date.clone().subtract(3, 'months').year(), 3, 1]);
